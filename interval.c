...
void ensure_capacity(IntervalSet *set) {
  if (set->size >= set->capacity) {
    ...
    Interval *new_intervals =
        realloc(set->intervals, new_capacity * sizeof(Interval));
...
void add_at_index(IntervalSet *set, Interval *interval) {
  ...
  for (index = set->size - 1; index >= 0; index--) {
    if (mpz_cmp(set->intervals[index].upper, interval->lower) < 0)
      break;
  }
...
void add_interval(IntervalSet *set, Interval *interval) {
  ...
  for (i = 0; i < set->size; i++) {
    if (in(&set->intervals[i], interval->lower) &&
        in(&set->intervals[i], interval->upper))
      return;
